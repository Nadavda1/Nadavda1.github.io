...
int writeAddressToBFile(FILE* fp, Address* pAdd)
{
	int len1, len2;
	len1 = (int)strlen(pAdd->city) + 1;
	len2 = (int)strlen(pAdd->street) + 1;

	if (fwrite(&pAdd->num, sizeof(int), 1, fp) != 1)
		return 0;
	fwrite(&len2, sizeof(int), 1, fp);
	if (fwrite(pAdd->street, sizeof(char), len2, fp) != len2)
		return 0;
	fwrite(&len1, sizeof(int), 1, fp);
	if (fwrite(pAdd->city, sizeof(char), len1, fp) != len1)
		return 0;

	return 1;
}
int readAddressToBFile(FILE* fp, Address* pAdd)
{
	int len1, len2;
	if (fread(&pAdd->num, sizeof(int), 1, fp) != 1)
		return 0;
	if (fread(&len2, sizeof(int), 1, fp) != 1)
		return 0;
	pAdd->street = (char*)malloc(len2 * sizeof(char));
	if (fread(pAdd->street, sizeof(char), len2, fp) != len2)
	{
		free(pAdd->street);
		return 0;
	}
	if (fread(&len1, sizeof(int), 1, fp) != 1)
		return 0;
	pAdd->city = (char*)malloc(len1 * sizeof(char));
	if (fread(pAdd->city, sizeof(char), len1, fp) != len1)
	{
		free(pAdd->street);
		free(pAdd->city);
		return 0;
